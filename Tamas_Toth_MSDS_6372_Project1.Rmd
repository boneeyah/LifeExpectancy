...
drop = c('HIV.AIDS', 'percentage.expenditure', 'Country',)
LifeExp = LifeExp[, !(colnames(LifeExp) %in% drop)]
...
#####################################################################################
library(leaps)
mlr.fwd=regsubsets(Life.expectancy~.,data=train,method="forward",nvmax=23)
testASE<-c()
for (i in 1:23){
...
mlr.fwd.final=regsubsets(Life.expectancy~.,data=LifeExp,method="forward",nvmax=23)
coef(mlr.fwd.final,24)
