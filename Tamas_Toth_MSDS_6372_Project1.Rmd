...
LifeExp2 <- LifeExp2 %>% dplyr::mutate(HIVcat = if_else(LifeExp2$HIV.AIDS>0.1,1,0),
                                       Meascat= if_else(LifeExp2$Measles>0,1,0))
LifeExp2$Meascat <- as.factor(LifeExp2$Meascat)
LifeExp2$HIVcat <- as.factor(LifeExp2$HIVcat)
#retrain/fit/test/predict and measure accuracy
train_cus <- LifeExp2[index,]
test_cu <- LifeExp2[index,]
fit.custom <- lm(Life.expectancy ~Year + Status + Adult.Mortality + infant.deaths + Alcohol + percentage.expenditure + Measles + BMI +  Polio + Total.expenditure + Diphtheria + Population + thinness.5.9.years + Income.composition.of.resources + Schooling + log.HIV.AIDS + log.GDP + Region + Meascat:Measles + HIVcat:log.HIV.AIDS +HIVcat +Meascat,data = train_cus)
#RMSE performs slightly better. 

train_cus_pred <- predict(fit.custom,test_cu)
accuracy(train_cus_pred,test_cu$Life.expectancy)
```
